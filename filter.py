...
>>> args
Namespace(database='/var/lib/mlocate/mlocate.db', limit_input=-1, limit_output=-1, log_level='WARNING', regexps=[], show_config=True)
>>> run(args)
...
def arg_parser():
    ...
    >>> parser.parse_args('--log-level CRITICAL'.split()) # doctest: +ELLIPSIS
    Namespace(...log_level='CRITICAL'...)

    Dry-run, show config only
    >>> parser.parse_args('--show-config'.split()) # doctest: +ELLIPSIS
